...
export class BoardCommentsService {
  ...
  async findAll(userId: string, postId: string, query: PaginationQueryDto) {
    ...
    const [comments, total] = await Promise.all([
      this.prisma.boardComment.findMany({
        where,
      ...
      this.prisma.boardComment.count({ where }),
    ]);

    // Gather all comment IDs (parent + child) for like check
    ...
    const userLikes = await this.prisma.boardLike.findMany({
      where: { userId, targetType: "comment", targetId: { in: allCommentIds } },
  ...
  async create(userId: string, postId: string, dto: CreateCommentDto) {
    // Verify post exists
    const post = await this.prisma.boardPost.findUnique({
      where: { id: postId, deletedAt: null, publishStatus: "published" },
    });
    if (!post) throw new NotFoundException("投稿が見つかりません");

    // If replying, verify parent comment exists
    if (dto.parentCommentId) {
      const parent = await this.prisma.boardComment.findUnique({
