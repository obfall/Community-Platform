...
describe("AlbumsController", () => {
  ...
  it("POST /albums/categories は service.createCategory を name で呼ぶ", async () => {
  ...
  it("POST /albums は service.create を userId と dto で呼ぶ", async () => {
  ...
  it("PATCH /albums/:id は service.update を id, body, currentUser で呼ぶ", async () => {
  ...
  it("POST /albums/:id/photos は service.addPhotos を albumId, userId, photos で呼ぶ", async () => {
    const photos = [{ fileId: TEST_FILE_ID, title: "夕焼け" }];
    const res = await request(app.getHttpServer())
      .post(`/albums/${TEST_ALBUM_ID}/photos`)
      .send({ photos });
    expect(res.status).toBe(201);
    expect(serviceMock.addPhotos).toHaveBeenCalledWith(TEST_ALBUM_ID, TEST_USER_ID, photos);
  });

  it("DELETE /albums/:id/photos/:photoId は 204 で service.removePhoto を呼ぶ", async () => {
    const res = await request(app.getHttpServer()).delete(
      `/albums/${TEST_ALBUM_ID}/photos/${TEST_PHOTO_ID}`,
    );
    expect(res.status).toBe(204);
    expect(serviceMock.removePhoto).toHaveBeenCalledWith(TEST_ALBUM_ID, TEST_PHOTO_ID);
  });
