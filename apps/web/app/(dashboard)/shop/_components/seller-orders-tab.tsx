...
import { useSellerOrders } from "@/hooks/shop/use-shop";
import { Card, CardContent } from "@/components/ui/card";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Receipt } from "lucide-react";
import { OrderStatusBadge } from "./order-status-badge";

// 販売者（＝現在のユーザー）宛ての注文一覧。manage（owner+）と seller（出品 member）で共用。
export function SellerOrdersTab({ backTo }: { backTo: "manage" | "seller" }) {
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => (
            <Link key={order.id} href={`/shop/orders/${order.id}?from=${backTo}`}>
