...
export default function EventsPage() {
  ...
  // 検索キーワードがタグ名・会場名にマッチするかの判定（pgroonga ヒット結果のうち
  // どこでマッチしたかを表示する目的。バック側で title 用の titleHighlighted は別途返ってくる）。
  const submittedSearch = query.search?.trim().toLowerCase() ?? "";
  const isMatched = (text: string | null | undefined) =>
    submittedSearch.length > 0 && text != null && text.toLowerCase().includes(submittedSearch);
                ...
                <CardContent className="p-4">
                    ...
                    <div className="mb-2 flex flex-wrap gap-1">
                      {event.tags.map((tag) => (
                        <Badge
                          ...
                          className={cn(
                            "text-xs",
                            isMatched(tag.name) && "bg-yellow-200 text-foreground",
                          )}
                        >
                          {tag.name}
                        </Badge>
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {isMatched(event.venueName) ? (
                        <span className="keyword">{event.venueName}</span>
                      ) : (
