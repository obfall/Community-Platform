import { useMutation, useQuery } from "@tanstack/react-query";
import { toast } from "sonner";
import { usageHistoryApi } from "@/lib/api/usage-history";
import type { ActivityLogQuery } from "@/lib/api/types";

export function useActivityLogs(params?: ActivityLogQuery) {
  return useQuery({
    queryKey: ["usage-history", "activity", params],
    queryFn: () => usageHistoryApi.listActivityLogs(params),
    staleTime: 30 * 1000,
  });
}

export function useExportActivityLogs() {
  return useMutation({
    mutationFn: (params?: ActivityLogQuery) => usageHistoryApi.exportActivityLogs(params),
    onSuccess: (data) => {
      const blob = new Blob([data as BlobPart], { type: "text/csv; charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const today = new Date().toISOString().split("T")[0]!.replace(/-/g, "");
      a.download = `activity-logs_${today}.csv`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success("操作ログを CSV でダウンロードしました");
    },
    onError: () => toast.error("CSV エクスポートに失敗しました"),
  });
}
