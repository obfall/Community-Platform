...
### 2026-05-03 (層4: hls.js 動的 import 適用後)

| Route                        |     Before |       After |           差分 |
| ---------------------------- | ---------: | ----------: | -------------: |
| `/videos/[id]` Page JS       |     169 kB | **13.8 kB** |           -92% |
| `/videos/[id]` First Load JS | **365 kB** |  **210 kB** | -155 kB / -42% |

`hls.js` を `next/dynamic` で遅延ロード化したことで、動画詳細ページの初期バンドルから
156 KB（gzip）が消え、必要時に別 chunk として後追いロードされるようになった。

### 2026-MM-DD (Phase 11.2 完了後)

層2〜層5 を全て適用した後の最終値を記録（更新時に書く）。
