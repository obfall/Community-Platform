...
| 5   | 掲示板             |          9 | board_categories, board_posts, board_post_attachments, board_post_tags, board_comments, board_likes, board_topics, board_topic_posts, board_topic_post_comments                                                                                                                                                            |
...
- **PK**: 全テーブル UUID v4、`gen_random_uuid()` で自動生成
- **共通カラム**: 更新系テーブルは `id`, `created_at`, `updated_at`、履歴・イベントログ系（login_histories, refresh_tokens, password_reset_tokens, notifications, point_histories, activity_logs, audit_logs, webhook_deliveries, advertisement_events 等）は `id`, `created_at` を基本とする
...
- **日本語全文検索**: pgroonga インデックス（USING pgroonga）を使用
...
| is_enabled         | BOOLEAN      | NOT NULL, DEFAULT TRUE, CHECK (category != 'common' OR is_enabled = TRUE) | 有効フラグ（common は常に TRUE、optional は管理者が切替）                                                                                                                             |
...
> **備考**: 上記は初期値の例。共通/オプションの分類は要件に応じて調整可能。`category = 'common'` の行は DB の CHECK 制約（`CHECK (category != 'common' OR is_enabled = TRUE)`）により `is_enabled = FALSE` への更新が防止される。アプリケーション層でも同様のバリデーションを行い、ユーザーに適切なエラーメッセージを返す。
...
| id          | UUID        | PK, DEFAULT gen_random_uuid()                                                   | ID         |
| user_id     | UUID        | FK → users.id, NOT NULL                                                         | ユーザーID |
| target_type | VARCHAR(20) | NOT NULL, CHECK IN ('post','comment','topic','topic_post','topic_post_comment') | 対象種別   |
...
### 5-6. `board_topics` — 掲示板トピック

カテゴリ内のトピック。トピックにはタイトルと本文（最初のメッセージ）があり、その下に投稿（投稿者からの返答）がぶら下がる構造。

| カラム名       | 型           | 制約                                                          | 説明                   |
| -------------- | ------------ | ------------------------------------------------------------- | ---------------------- |
| id             | UUID         | PK, DEFAULT gen_random_uuid()                                 | ID                     |
| category_id    | UUID         | FK → board_categories.id, NOT NULL                            | カテゴリID             |
| author_user_id | UUID         | FK → users.id, NOT NULL                                       | 作成者ユーザーID       |
| title          | VARCHAR(200) | NOT NULL                                                      | トピックタイトル       |
| body           | TEXT         | NOT NULL                                                      | トピック本文           |
| publish_status | VARCHAR(20)  | NOT NULL, DEFAULT 'published', CHECK IN ('draft','published') | 公開ステータス         |
| is_pinned      | BOOLEAN      | NOT NULL, DEFAULT FALSE                                       | ピン留めフラグ         |
| view_count     | INTEGER      | NOT NULL, DEFAULT 0                                           | 閲覧数                 |
| post_count     | INTEGER      | NOT NULL, DEFAULT 0                                           | 投稿数（キャッシュ）   |
| like_count     | INTEGER      | NOT NULL, DEFAULT 0                                           | いいね数（キャッシュ） |
| created_at     | TIMESTAMPTZ  | NOT NULL, DEFAULT NOW()                                       | 作成日時               |
| updated_at     | TIMESTAMPTZ  | NOT NULL, DEFAULT NOW()                                       | 更新日時               |
| deleted_at     | TIMESTAMPTZ  | NULL                                                          | 論理削除日時           |

**インデックス**: idx ON (category_id, is_pinned DESC, created_at DESC), idx ON (author_user_id)

---

### 5-7. `board_topic_posts` — トピック投稿

トピックに対する投稿。フラット構造（ネストなし）。将来的に画像・添付ファイル対応予定。

| カラム名       | 型          | 制約                                    | 説明                     |
| -------------- | ----------- | --------------------------------------- | ------------------------ |
| id             | UUID        | PK, DEFAULT gen_random_uuid()           | ID                       |
| topic_id       | UUID        | FK → board_topics.id, NOT NULL, CASCADE | トピックID               |
| author_user_id | UUID        | FK → users.id, NOT NULL                 | 投稿者ユーザーID         |
...
**インデックス**: idx ON (topic_id, created_at), idx ON (author_user_id)

---

### 5-8. `board_topic_post_comments` — トピック投稿コメント（返信）

トピック投稿に対するコメント。ネスト構造対応（parent_comment_id による自己参照）。

| カラム名          | 型          | 制約                                         | 説明                     |
| ----------------- | ----------- | -------------------------------------------- | ------------------------ |
| id                | UUID        | PK, DEFAULT gen_random_uuid()                | ID                       |
| post_id           | UUID        | FK → board_topic_posts.id, NOT NULL, CASCADE | 投稿ID                   |
| author_user_id    | UUID        | FK → users.id, NOT NULL                      | コメント者ユーザーID     |
| parent_comment_id | UUID        | FK → board_topic_post_comments.id, NULL      | 親コメントID（ネスト用） |
| body              | TEXT        | NOT NULL                                     | コメント本文             |
| like_count        | INTEGER     | NOT NULL, DEFAULT 0                          | いいね数（キャッシュ）   |
| created_at        | TIMESTAMPTZ | NOT NULL, DEFAULT NOW()                      | 作成日時                 |
| updated_at        | TIMESTAMPTZ | NOT NULL, DEFAULT NOW()                      | 更新日時                 |
...
**インデックス**: idx ON (post_id, created_at), idx ON (parent_comment_id), idx ON (author_user_id)
...
動画管理。シリーズ分け、閲覧制限、視聴トラッキング対応。配信基盤に依存しない抽象化設計（Cloudflare Stream ↔ R2+自前HLS の移行が可能）。
...
**インデックス**: idx ON (publish_status, sort_order) WHERE deleted_at IS NULL, idx ON (series_id), idx ON (series_id, watch_order)
...
| id          | UUID         | PK, DEFAULT gen_random_uuid()                                                                                                                                       | ID                          |
| user_id     | UUID         | FK → users.id, NOT NULL                                                                                                                                             | ユーザーID                  |
| source_type | VARCHAR(30)  | NULL, CHECK IN ('event','project_task','skill_booking'), CHECK ((source_type IS NULL AND source_id IS NULL) OR (source_type IS NOT NULL AND source_id IS NOT NULL)) | 連携元種別（NULL=個人予定） |
...
**インデックス**: idx ON (user_id, start_at, end_at) WHERE deleted_at IS NULL, UNIQUE ON (user_id, source_type, source_id) WHERE source_type IS NOT NULL, idx ON (source_type)
...
| text_value       | TEXT        | NULL                                | テキスト値（attribute_type='text'用）                                   |
| numeric_value    | NUMERIC     | NULL                                | 数値（attribute_type='number'用）                                       |
| date_value       | DATE        | NULL                                | 日付値（attribute_type='date'用）                                       |
| selected_options | JSONB       | NULL                                | 選択値（attribute_type='select','multi_select'用: ["value1","value2"]） |
| created_at       | TIMESTAMPTZ | NOT NULL, DEFAULT NOW()             | 作成日時                                                                |
| updated_at       | TIMESTAMPTZ | NOT NULL, DEFAULT NOW()             | 更新日時                                                                |

**インデックス**: UNIQUE ON (user_id, attribute_id), idx ON (attribute_id), idx ON (attribute_id, numeric_value), idx ON (attribute_id, date_value), GIN ON (selected_options)

**備考**: `member_attributes.attribute_type` に応じて、対応する1カラムのみを設定する（text→text_value、number→numeric_value、date→date_value、select/multi_select→selected_options）。排他制御はアプリケーション層（Zodバリデーション + サービス層）で強制し、他カラムは NULL とする。
...
6. **日本語全文検索**: `board_posts.body`, `products.name` 等は pgroonga インデックス（USING pgroonga）で検索対応
7. **UUID v4**: `gen_random_uuid()` で自動生成。Supabase・Prisma 標準対応
8. **マルチコミュニティ拡張**: 将来的に複数コミュニティ対応が必要な場合は、`communities` テーブルを追加し各テーブルに `community_id` FK を付与することで対応可能
9. **機能設定（共通/オプション）**: `feature_settings` テーブルで各機能モジュールの有効/無効を管理。`category = 'common'` の機能は DB CHECK 制約（`CHECK (category != 'common' OR is_enabled = TRUE)`）により無効化を防止し、アプリケーション層でも同様のバリデーションを行う。オプション機能を無効にした場合、該当モジュールの画面はメニューから非表示となり、APIはアクセス拒否（403）を返す。既存データは保持され、再有効化時に復元される
